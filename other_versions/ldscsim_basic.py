...
def ascertainment_bias(mt,y,P):
    '''Adds ascertainment bias to a binarized phenotype.
    
    Parameters
    ----------
    mt : :class:`.MatrixTable`
        MatrixTable containing phenotype to be used.
    y : :class:`.Expression`
        Column field of binary phenotype.
    P : :obj:`int` or :obj:`float`
        Prevalence of binarized phenotype in sample.
    '''
    assert P>=0 and P<=1, 'P must be in [0,1]'
    tid = ''.join(random.choices(string.ascii_uppercase + string.ascii_lowercase, k=5)) # "temporary id" -- random string to identify temporary intermediate fields generated by this method
    mt = mt.annotate_cols(**{'y_'+tid:y})
    y_stats = mt.aggregrate_cols(hl.agg.stats(mt['y_'+tid]))
    K = y_stats.mean
    assert abs(P-K)<1, 'Prevalence in population is incompatible with specified prevalence for sample.'
    p = (1-K)*P/(K*(1-P))
    if P < K:
        con = mt.filter_cols(mt['y_'+tid]==0) 
        cas = mt.filter_cols(mt['y_'+tid]==1).sample_cols(p=p)
        mt = cas.union_cols(con)
    elif P > K:
        cas = mt.filter_cols(mt['y_'+tid]==1)
        con = mt.filter_cols(mt['y_'+tid]==0).sample_cols(p=p**(-1)) 
        mt = cas.union_cols(con)
    mt = clean_fields(mt, tid)
    return mt
...
def binarize(mt,y,K):
    ...
    Uses inverse CDF to set binarization threshold.'''

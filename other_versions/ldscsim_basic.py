...
def make_betas(mt, h2, pi=1, annot=None, rg=None):
    ...
    if annot != None: #annotation-informed
        mt = mt._annotate_all(row_exprs={'annot_'+tid:annot})
        mt = mt.annotate_rows(beta = hl.rand_norm(0, hl.sqrt(mt['annot_'+tid]))) # if is_h2_normalized: scale variance of betas to be h2, else: keep unscaled variance
...
#    mt = mt._annotate_all(entry_exprs={'gt_'+tid:genotypes})
    mt = mt.annotate_entries(**{'gt_'+tid: genotypes})
#    mt = mt._annotate_all(row_exprs={'gt_stats_'+tid:hl.agg.stats(mt['gt_'+tid])})

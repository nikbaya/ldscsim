...
def make_betas(mt, h2, pi=None, annot=None, rg=None):
    ...
    # multi-trait correlated infinitesimal
        ...
        mt, pi, rg = multitrait_ss(mt=mt, h2=h2, rg=0 if rg is [
                                   None] else rg[0], pi=pi)
...
def multitrait_inf(mt, h2=None, rg=None, cov_matrix=None, seed=None):
    ...
    """
    # tid = "temporary id" -- random string to identify temporary intermediate fields generated by this method
    tid = ''.join(random.choices(string.ascii_uppercase + string.ascii_lowercase, k=5)
                  )
    h2 = h2.tolist() if type(h2) is np.ndarray else (
        [h2] if type(h2) is not list else h2)
    rg = rg.tolist() if type(rg) is np.ndarray else (
    ...
    assert h2 is not [
        None] or cov_matrix is not None, 'h2 and cov_matrix cannot both be None'
...
def multitrait_ss(mt, h2, pi, rg=0, seed=None):
    ...
    beta = randstate.multivariate_normal(
        mean=np.zeros(2), cov=cov_matrix, size=[int(M), ])
...
def normalize_genotypes(genotype):
    ...
    mt = mt.annotate_entries(
        norm_gt=(mt['gt_'+tid]-mt['gt_stats_'+tid].mean)/mt['gt_stats_'+tid].stdev)
    mt = _clean_fields(mt, tid)
    return mt

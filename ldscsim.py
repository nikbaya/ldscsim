...
def simulate_phenotypes(mt, genotype, h2, pi=None, rg=None, annot=None, popstrat=None,popstrat_var=None):
    ...
    :class:`.MatrixTable`
        ...
        to specified model.
    """
    h2 = h2.tolist() if type(h2) is np.ndarray else h2
    pi = pi.tolist() if type(pi) is np.ndarray else pi
    rg = rg.tolist() if type(rg) is np.ndarray else rg    
...
def multitrait_inf(mt, h2=None, rg=None, cov_matrix=None, seed=None):
    ...
    if cov_matrix != None:
        ...
        cov, rg = create_cov_matrix(h2, rg)
    cov = (1/M)*cov
...
def create_cov_matrix(h2, rg, psd_rg=False):
    ...
    if not np.all(np.linalg.eigvals(cov_matrix) >=  0) and not psd_rg: #check positive semidefinite
        ...
        cov_matrix, rg = create_cov_matrix(h2, rg, psd_rg=True)
